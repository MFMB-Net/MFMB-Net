...
'''
NHFnet｜cgate |psmixer
mosi mosei sims需要改变seq维度：183-203 self.seq_v_[dataset]改，并且sims的alignment25改成1
不是前后衔接的，而是alignment、generator之后的数据分别走两个分支，最后concat【nhf的聚合块｜cgate】进入classifier
1、self.embed_dim=90#mosi 是从alignment出来的dim数值，因为原本dim=30，concat了tt ta tv注意力的结果变成了3*dim
2、这个token，我对vat都吸取了一遍，[audio,fsn]-[fsn',visual]-[fsn'',text]-----[fsn''/'
其实还可以改进，因为alignment出来的是3d的数据，vv｜va｜vt，有冗余。最好是v a t单独，但是融合在fusion_1里面不好改，要单独写个文件好改
3、classfier里的参数，我写死了+90这个值，不同数据集不一定一样，最好是从args里取，，，不会写
并且90的话参数有点大，后续可以调整linear降维度
4、150行：统一句子长度到和t一样，保证可以和fsn有序拼接。应该mosi mosei=50 sims=39




3、分进入classficiation有两个情况，一个是psmixer特征+tokens接进入 classifier1 ，
                               一个是psmixer特征也和cgate特征拼接+tokens进入 classifier2
'''
...
class BottleAttentionNet(nn.Module):
    ...
    def forward(self,audio,visual,text):#50,24,90
        #降维度90-32
        ...
        fsn=torch.zeros(self.seq,audio.size(1),self.embed_dim).cuda()#4，24，90/32

        x=torch.cat([audio,fsn],dim=0)#54,24,32

        for i in range(self.layer_multimodal):
            if i==0:
                #[audio,fsn]-[fsn',visual]-[fsn'',text]-----[fsn''']
                x=self.transformer(x) #54，24，32
                x=torch.cat([x[audio.size(0):,:,:],visual],dim=0)#54，24，32
        ...
        x=x[:self.seq,:,:]#4，24，90这里直接取得四个，取50也可以，反正之后只用x【-1】后两维
        return x
...
class GATE_F(nn.Module):
    def __init__(self, args):
        ...
        # nhf统一句子长度到和t一样，应该都是mosi mosei=50 sims=39
        #1.针对mosi
        self.seq_v_mosi=nn.Linear(500,50)
        self.seq_a_mosi=nn.Linear(375,50)
        
        #2.针对sims
        self.seq_t_sims=nn.Linear(39,50)
        self.seq_v_sims=nn.Linear(55,50)
        self.seq_a_sims=nn.Linear(400,50)
        ...
        self.MLP_Communicator1=MLP_Communicator(self.dim,2,hidden_size=64,depth=1)#hiddensize超参数 可以改变
        self.MLP_Communicator2 = MLP_Communicator(self.dim, 2, hidden_size=64, depth=1)
        self.args=args
        self.batch_size=self.args.batch_size#不会拿 不会写
        #self.batch_size=24 #写死
        #mosi mosei sims堆叠要相同长度 mosi写死
        ...
        # 1.psmixer堆叠+token的直接classification
    ...
    def forward(self, text_x, audio_x, vision_x):
        ...
        #只有sims需要text_x_fusion=self.seq_t_sims
        #text_x_fusion=self.seq_t_sims(text_x.permute(0,2,1))#24,90,375--24,90,50-
        ...
        #24,64      batchsize,common_size 长度相同的向量条才可以堆叠
        ...
        #调整维度 适应分类
        #stack_rep_tv = stack_rep_tv.reshape(24, -1)#batchsize写死
        stack_rep_tv=stack_rep_tv.reshape(self.batch_size,-1)#24,128
